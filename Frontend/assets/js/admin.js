...
const UserActions = {
    loadUsers: function() { UserService.getAll(UI.renderUsersTable, (err) => toastr.error("Failed to load users.")); },
    ...
    handleFormSubmit: function(e) { e.preventDefault(); const userId = $("#userId").val(); const userData = { username: $("#username").val(), email: $("#email").val(), first_name: $("#firstName").val(), last_name: $("#lastName").val(), role: $("#role").val() }; if (userId) { UserService.updateUser(userId, userData, () => { bootstrap.Modal.getInstance(document.getElementById('userModal')).hide(); toastr.success("User updated successfully."); UserActions.loadUsers(); }, (error) => { toastr.error("Update failed: " + (error.error || "Unknown error.")); }); } else { userData.password = $("#password").val(); if (!userData.password) { toastr.warning("Password is required for new users."); return; } UserService.addUser(userData, () => { bootstrap.Modal.getInstance(document.getElementById('userModal')).hide(); toastr.success("User added successfully."); UserActions.loadUsers(); }, (error) => { toastr.error("Add failed: " + (error.error || "Unknown error.")); }); } },
...
const CourseActions = {
    loadCourses: function() { CourseService.getAll(null, UI.renderCoursesTable, (err) => toastr.error("Failed to load courses.")); },
    ...
    handleFormSubmit: function(e) { e.preventDefault(); const courseId = $("#courseId").val(); const courseData = { title: $("#courseTitle").val(), description: $("#courseDescription").val(), instructor_id: $("#courseInstructorId").val(), price: $("#coursePrice").val(), difficulty_level: $("#courseDifficulty").val(), image_url: "" }; if (courseId) { CourseService.updateCourse(courseId, courseData, () => { bootstrap.Modal.getInstance(document.getElementById('courseModal')).hide(); toastr.success("Course updated successfully."); CourseActions.loadCourses(); }, (error) => toastr.error("Update failed: " + (error.error || "Unknown error."))); } else { CourseService.addCourse(courseData, () => { bootstrap.Modal.getInstance(document.getElementById('courseModal')).hide(); toastr.success("Course added successfully."); CourseActions.loadCourses(); }, (error) => toastr.error("Add failed: " + (error.error || "Unknown error."))); } },
    handleDelete: function() {
    ...
    enrollInCourse: function() { const user = AuthService.getUser(); if (!user) { toastr.warning("Please log in to enroll in a course."); window.location.hash = "#login"; return; } const courseId = window.selectedCourseId; if (!courseId) { toastr.error("Could not determine the course. Please try again."); return; } const enrollmentData = { user_id: user.id, course_id: courseId }; EnrollmentService.create(enrollmentData, (response) => { toastr.success("Congratulations! You have successfully enrolled in this course."); CourseActions.checkEnrollmentStatus(); }, (error) => { toastr.error("Enrollment failed: " + (error.error || "An unknown error occurred.")); }); },
        unenrollFromCourse: function(enrollmentId) {
        const confirmBtn = $("#confirmDeleteBtn");
        const modalInstance = new bootstrap.Modal(document.getElementById('confirmDeleteModal'));

        $("#confirmDeleteModalLabel").text("Confirm Unenrollment");
        $("#confirmDeleteModal .modal-body").text("Are you sure you want to unenroll from this course?");

        confirmBtn.off('click').one('click', function() {
            EnrollmentService.delete(enrollmentId, 
                () => {
                    toastr.info("You have been unenrolled.");
                    CourseActions.checkEnrollmentStatus();
                    modalInstance.hide();
                }, 
                (err) => {
                    toastr.error("Failed to unenroll.");
                    modalInstance.hide();
                }
            );
        });
...
const InstructorActions = {
    loadInstructors: function() { InstructorService.getAll(UI.renderInstructorsTable, (err) => toastr.error("Failed to load instructors.")); },
